...
public:
	...
	FibNode(T value,int node) :key(value), to(node), degree(0), marked(false),
...
public:
	...
	FibNode<T>* insert(T key, int node);
	// 移除斐波那契堆中的最小节点
	void removeMin();
...
private:
	...
	// 将节点node插入到斐波那契堆中
	void insertnode(FibNode<T>* node);
	// 将"堆的最小结点"从根链表中移除，
 ...
 */
template <class T>
FibNode<T>* FibHeap<T>::insert(T key,int to)
{
	FibNode<T>* node;

	node = new FibNode<T>(key,to);
	if (node == NULL)
		return NULL;
